...
def upsample(in_channels, out_channels, upscale, kernel_size=3, versionmode = 0):
    layers = []
    # The main encoder
    if versionmode == 0:
      ...
      last_conv = nn.Sequential(       SeparableConv2d(in_channels,96,3,1,1),
                                        nn.BatchNorm2d(96),
                                        nn.ReLU(),
                                        SeparableConv2d(96,96,3,1,1),
                                        nn.BatchNorm2d(96),
                                        nn.ReLU(),
                                        nn.Conv2d(96, out_channels, kernel_size=1, stride=1))
      layers.append(last_conv[0])
      layers.append(last_conv[1])
      layers.append(last_conv[2])
      layers.append(last_conv[3])
      layers.append(last_conv[4])
      layers.append(last_conv[5])
      layers.append(last_conv[6])
...
class MainDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, num_classes, versionmode=0):
        super(MainDecoder, self).__init__()
        self.upsample = upsample(conv_in_ch, num_classes, upscale=upscale, versionmode = versionmode)
...
class DropOutDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, num_classes, drop_rate=0.3, spatial_dropout=True, versionmode=0):
...
class FeatureNoiseDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, num_classes, uniform_range=0.3, versionmode=0):
...
class VATDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, num_classes, xi=1e-1, eps=10.0, iterations=1,versionmode=0):
...
class CutOutDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, num_classes, drop_rate=0.3, spatial_dropout=True, erase=0.4, versionmode=0):
...
class ContextMaskingDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, num_classes, versionmode):
